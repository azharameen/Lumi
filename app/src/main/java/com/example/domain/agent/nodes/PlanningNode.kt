...
class PlanningNode(
    ...
    override suspend fun execute(state: AgentState): AgentState {
        // If state is local execution or local on-device engine is ready, do not call Cloud Firebase AI
        if (state.isLocalExecution || onDeviceGemmaEngine?.isModelReady() == true) {
            return state.copy(
                currentThought = "Local-first strategy activated: directly executing without cloud roundtrip."
            )
        }
